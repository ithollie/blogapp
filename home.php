...
$users   =  mysqli_query($database->conn ,"SELECT * FROM `register`  WHERE  email='".$_SESSION['email']."'" ) or die($database->conn->error);

$blogssql   =  mysqli_query($database->conn , "SELECT * FROM  `blogs` WHERE  email='".$_SESSION['email']."' ORDER  BY  `title`  LIMIT  4") or die($database->conn->error);

$postss   =  mysqli_query($database->conn,   "SELECT *  FROM `blogs` WHERE  email='".$_SESSION['email']."' ORDER  BY  `title`") or die($database->conn->error);
            ...
            <div id="content" style="height:500px; font-size:24px; text-align:center">
                ...
                <?php while($row = mysqli_fetch_array($blogssql, MYSQLI_ASSOC)){ ?>
                        ...
                        <div id="<?php echo $row['id'] ?>" >
                          ...
                          <img style="width:1200px; height:700px;"  src="<?php echo 'uploads/'.$row['postImage'] ?>" alt="" class=" responsive-img">  
